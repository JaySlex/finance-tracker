...
export default function Accounts() {
  ...
  const saveEdit = i => {
    ...
    setAccounts(accounts.map((acc, idx) => idx === i ? { ...acc, balance: Number(editBalance) } : acc));
    setEditIndex(null);
    setEditBalance("");
  };
      ...
      {accounts.length > 0 && (
        <div className="flex items-baseline gap-6 mb-4">
          <span className="text-xl font-bold text-black">
            Total Accounts: ${accountsTotal.toLocaleString("en-CA", { minimumFractionDigits: 2 })}
          </span>
        </div>
      )}
      {globalEdit && (
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          <input className="border rounded-xl px-3 py-2" placeholder="Balance" type="number" value={balance} onChange={e => setBalance(e.target.value)} />
          <button
            ...
            onClick={() => {
              ...
              setAccounts([...accounts, { name, balance: Number(balance) }]);
              setName(""); setBalance("");
            }}
          >
            Add
            ...
            <li key={i} className="flex justify-between items-center p-2 border-b gap-2 text-sm">
              ...
              {editIndex === i ? (
                ...
                <div className="flex items-center">
                  <span>${a.balance.toLocaleString("en-CA", { minimumFractionDigits: 2 })}</span>
                  {globalEdit && (
                    <>
                      <button
                        className="ml-3 bg-blue-500 text-white rounded-xl px-3 py-1"
                        onClick={() => startEdit(i, a.balance)}
                      >
                        Edit
                      </button>
                      <button
